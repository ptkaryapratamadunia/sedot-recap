...
if uploaded_files:  # Jika user telah memilih file
    ...
    # Menambahkan baris 'Category' dengan isi 'MOR'
    mor_table.loc['Category'] = ''
    mor_table.loc['Category', 'Nama File'] = 'Category'
    for col in header_names:
        mor_table.loc['Category', col] = 'MOR'
    ...
    with tab_qty:#Tab untuk Qty
        ...
        st.dataframe(qty_table, use_container_width=True,hide_index=True).applymap(lambda x: f"{int(x):,}" if isinstance(x, (int, float)) else x)  # Format Qty tanpa desimal
